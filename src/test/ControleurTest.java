...
public class ControleurTest {
	...
	public void testGererFichierPlan() {
		...
		List<Troncon> troncons = new ArrayList<Troncon>(c.getPlan()
				.getToutTroncons());
	...
	public void testGererFichierPlanNonXML() {
		...
		Controleur c = new Controleur();
		c.setModeTest(true);
		assertEquals(Codes.PARSE_ERROR,
	...
	public void testGererFichierPlanNoeudDestInexistant() {
		...
		Controleur c = new Controleur();
		c.setModeTest(true);
		assertEquals(Codes.ERREUR_301,
				c.gererFichier(new File("XML/errors/plan301.xml"), "plan"));
		assertEquals(0, c.getPlan().getToutNoeuds().size());
		assertEquals(0, c.getPlan().getToutTroncons().size());
	}
	...
	public void testGererFichierPlanMauvaiseSpecTroncon() {
		...
		Controleur c = new Controleur();
		c.setModeTest(true);
		assertEquals(Codes.ERREUR_302,
				c.gererFichier(new File("XML/errors/plan302.xml"), "plan"));
		assertEquals(0, c.getPlan().getToutNoeuds().size());
		assertEquals(0, c.getPlan().getToutTroncons().size());
	}
	...
	public void testGererFichierPlanMauvaiseSpecNoeud() {
		...
		Controleur c = new Controleur();
		c.setModeTest(true);
		assertEquals(Codes.ERREUR_303,
				c.gererFichier(new File("XML/errors/plan303.xml"), "plan"));
		assertEquals(0, c.getPlan().getToutNoeuds().size());
		assertEquals(0, c.getPlan().getToutTroncons().size());
	}
	...
	public void testGererFichierLivraisons() {
		...
		assertNotNull("noeud de l'entrepot null", c.getTournee().getEntrepot()
				.getNoeud());
	...
	public void testGererFichierLivraisonsVide() {
		...
		assertEquals(new ArrayList<PlageHoraire>(), c.getTournee()
				.getPlagesHoraires());
	}
	...
	public void testGererFichierLivraisonsPlagesChevauchent() {
		...
		// c.setModeTest(true);
		c.gererFichier(new File("XML/plan2.xml"), "plan");
		c.gererFichier(new File("XML/errors/livraisonsChevauche.xml"),
				"horaires");
		...
		assertEquals("Liste de plages horaires pas de la bonne taille...", 1, c
				.getTournee().getPlagesHoraires().size());
		assertEquals("Il ne reste pas la bonne plage horaire...",
				plage.getHeureDebut(), c.getTournee().getPlagesHoraires()
						.get(0).getHeureDebut());
		assertEquals("Il ne reste pas la bonne plage horaire...",
				plage.getHeureFin(), c.getTournee().getPlagesHoraires().get(0)
						.getHeureFin());
	...
	public void testGererFichierLivraisonsEntrepotInvalide() {
		...
		assertEquals(Codes.ERREUR_306, c.gererFichier(new File(
				"XML/errors/livraisonsEntrepotFaux.xml"), "horaires"));
	}
	...
	public void testGererFichierLivraisonsPlageHoraireInvalide() {
		Controleur c = new Controleur();
		c.gererFichier(new File("XML/plan2.xml"), "plan");
		c.gererFichier(new File("XML/errors/livraisonsPlageInvalide.xml"),
				"horaires");

		assertEquals("Il n'y a pas qu'une seule plage...", 1, c.getTournee()
				.getPlagesHoraires().size());
		assertEquals("Il n'y a pas qu'une seule demande...", 1, c.getTournee()
				.getPlagesHoraires().get(0).getDemandeLivraison().size());
		assertEquals(
				"La demande n'a pas le bon ID...",
				1,
				new ArrayList<DemandeDeLivraison>(c.getTournee()
						.getPlagesHoraires().get(0).getDemandeLivraison()).get(
						0).getId());
	}
	...
	public void testGererFichierLivraisonsLivraisonInvalide() {
		Controleur c = new Controleur();
		c.gererFichier(new File("XML/plan2.xml"), "plan");
		c.gererFichier(new File("XML/errors/livraisonsLivraisonInvalide.xml"),
				"horaires");

		DemandeDeLivraison demande = new DemandeDeLivraison(1, c.getPlan()
				.getNoeud(3), 621, null);
		assertEquals(1, c.getTournee().getPlagesHoraires().size());
		assertEquals(1, c.getTournee().getPlagesHoraires().get(0)
				.getDemandeLivraison().size());

		List<DemandeDeLivraison> demandes = new ArrayList<DemandeDeLivraison>(c
				.getTournee().getPlagesHoraires().get(0).getDemandeLivraison());

...
public class VueItineraire {
	 ...
	 * @param couleur
	 */
	public VueItineraire(Itineraire itineraire,
			HashMap<Integer, VueNoeud> vueNoeuds, int idPremierNoeud,
			String couleur) {
		...
		for (Troncon troncon : itineraire.getTronconsItineraire()) {
			vuesTroncons.add(new VueTroncon(noeudPrecedent, vueNoeuds
					.get(troncon.getNoeudFin().getId()), couleur, EPAISSEUR));

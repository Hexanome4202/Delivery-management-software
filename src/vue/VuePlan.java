...
public class VuePlan extends mxGraphComponent {
	...
	public static final String[] COULEUR_BORDURE = { "#838383", "#2d0968",
	...
	public void setPlan(Plan plan) {
		...
		for (Noeud noeud : plan.getToutNoeuds()) {
			vueNoeuds.put(noeud.getId(), new VueNoeud(noeud, hX, hY));
		}

		for (Noeud noeud : plan.getToutNoeuds()) {
			for (Troncon troncon : noeud.getTronconSortants()) {
				vueTroncons.add(new VueTroncon(vueNoeuds.get(noeud.getId()),
	...
	/**
	 * Méthode permettant de vider l'attribut gardant en mémoire le noeud à
	 * ajouter
	 */
	...
	/**
	 * Méthode permettant de récupérer le noeud de livraison sélectionné
	 * 
	...
	/**
	 * Méthode permettant de calculer les facteurs de mise à l'échelle à partir
	 * du xMax et yMax réels des points à afficher
	 * 
	 * @param xMax
	 * @param yMax
	 */
	private void calculeFacteurEchelle(int xMax, int yMax) {
		hY = (getSize().getHeight() - 20) / yMax;
		hX = (getSize().getWidth() - 20) / xMax;
	}
	 ...
	 * @return true si on est en phase d'ajout et le point sélectionné est une
	 *         demande de livraison false sinon
	 */
	...
	/**
	 * Methode permettant de savoir si le bouton Supprimer doit être activé ou
	 * non
	 * 
	 * @param noeud
	 *            le noeud actuellement sélectionné
	...
	/**
	 * Méthode permettant de préciser à la VuePlan que la tournee n'est plus
	 * dessinée
	 */

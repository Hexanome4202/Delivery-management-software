...
public class DemandeDeLivraison implements Comparable<DemandeDeLivraison> {
	...
	/**
	 * Permet de sauvegarder l'ID max afin de générer des IDs cohérents et non
	 * utilisés lors de l'ajout de livraisons
	 */
	...
	public DemandeDeLivraison(int id, Noeud noeud, int client,
		...
		this.id = id;
		if (id > DemandeDeLivraison.maxId)
			DemandeDeLivraison.maxId = id;
	 ...
	 * 
	 * @param entrepot
	 *            <code>Noeud</code> correspondant à l'entrepot
	 */
	...
	public int compareTo(DemandeDeLivraison demande) {
		if (demande != null && this.id == demande.getId()) {
			if (this.plageHoraire != null && demande.getPlage() != null) {
				if (this.plageHoraire.getHeureDebut().getTime() == demande
						.getPlage().getHeureDebut().getTime()) {
					return 0;
				}
				return 1;
			}
